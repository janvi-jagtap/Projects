...
//    int pageSize;
    ...
    */
    public void rollback(TransactionId tid) throws IOException {
        synchronized (Database.getBufferPool()) {
            synchronized (this) {
                ...
                //First get to pointer and seek to the start of the record
                    raf.seek(cur - LONG_SIZE);
                    long recStart = raf.readLong();
                    raf.seek(recStart);
    ...
    */
    public synchronized void shutdown() {
        try {
            logCheckpoint();  //simple way  is to write a checkpoint record
            raf.close();
        } catch (IOException e) {
    ...
    public void recover() throws IOException {
        synchronized (Database.getBufferPool()) {
            synchronized (this) {
                ...
                Map<Long, List<Long>> txnUpdateOffsets = new HashMap<>();
                ...
                while (raf.getFilePointer() < raf.length()) {
                    ...
                    try {
                        ...
                        if (type == UPDATE_RECORD) {
                            txnUpdateOffsets.computeIfAbsent(tid, k -> new ArrayList<>()).add(recordStart);
                            readPageData(raf);
                            Page after = readPageData(raf);
                            // REDO
                ...
                // UNDO loser xactions in reverse order
                for (Long tid : loser) {
                    List<Long> updates = txnUpdateOffsets.get(tid);
                    if (updates == null) continue;

                    ListIterator<Long> it = updates.listIterator(updates.size());
                    while (it.hasPrevious()) {
                        long offset = it.previous();
                        raf.seek(offset);
                        raf.readInt();
                        raf.readLong();
                        Page before = readPageData(raf);
